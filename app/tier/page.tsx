...
type TierContainerTypes = {
  [key: string]: Array<TierPlayerTypes>
}
...
export default function TierPage() {
  ...
  const [items, setItems] = useState<TierContainerTypes>({
    s: [],
    a: [],
    b: [],
    c: [],
  ...
  const onDragEnd = ({ source, destination }: DropResult) => {
    ...
    const scourceKey = source.droppableId as string
  ...
  useEffect(() => {
    if (data) {
      setItems({
        ...items,
        players: [
          ...data[position].map((el: PlayerTypes, i: number) => ({
            id: `${i + 1}`,
            ...el,
          })),
        ],
